    ...
    if "action" in activity and "time" in activity:
        ...
        with lock:
            if action in stats:
                ...
                stats[action]["avg"] = (prev_stat["avg"] * prev_stat["count"] + activity["time"]) / (prev_stat["count"] + 1)
...
def getStats() -> str:
    global stats, lock
    with lock:
        summary = [{"action": action, "avg": round(stat["avg"], 2)} for action, stat in stats.items()]
